...
/*   Updated: 2019/02/26 17:48:39 by acarlson         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

static int32_t		calc_point_2(t_vect3 *z, unsigned iters, int clr, int rot)
{
	t_vect3			old;
	t_vect3			new;
	unsigned		i;
...
void				*calc_mandelbrot(t_targ *p)
{
	...
	while (x < p->f->windowheight)
	{
		...
		while (y < p->end_y)
		{
			...
			PLT(p->f->img, (p->f->arg == 2 ? calc_point_2(n, p->f->iters,
				p->f->colors, p->f->rot) : calc_point_n(n, p->f)),
